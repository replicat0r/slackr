defmodule Slackr.SessionController do
 ...
 def create(conn, params) do
   case authenticate(params) do
    {:ok, user} ->
      ...
      new_conn |> put_status(:created) |> render ("show.json", user: user, jwt: jwt)
    :error -> 
 ...
 def delete(conn,_) do
  ...
  conn |> put_status(:ok) |> render("delete.json")
 end
 ...
 defp check_password(user, password) do
  case user do
   ...
   _ -> Comeonin.Bcrypt.check_password(password, user.password_hash)
  end
 end
 
 defp authenticate(%{"email"=>email, "password"=>password}) do
  user = Repo.get_by(Sling.User, emai: String.downcase(email))
